...
- Música del menú (`audio/music.js`, `makeMenu`): secuenciador de semicorcheas a 133,33 BPM enganchado en fase al reloj de la hoja KDZ (`LD.Main.kdz.rawTime()`): retícula anclada al acto III (beat b ↔ act3 + 0,45·b, b ∈ [−21, 22]), impacto en `lead`, acentos exactos en los cues fuera de retícula (slab, zoom, banda D, paneles, barrido de salida). Ocho ciclos de macroestructura (acorde, raíz del bajo, densidad, arpegios; ciclo 4 = breakdown), por lo que la pieza se repite cada 160 s. Estima la velocidad de la hoja y pasa a un reloj interno si la animación se detiene (pestaña oculta, movimiento reducido, sin WebGL). Herramientas: `tools/musictest.mjs` (niveles, traza, reloj real), `tools/musicsync.mjs` (precisión del planificador con reloj sintético: error medio 0,1 ms), `tools/musiclevels.mjs` (picos pre/post clipper y en el bus por acto).
